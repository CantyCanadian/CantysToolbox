...
public class SerializableCallbackDrawer : PropertyDrawer
{
    public override void OnGUI(Rect positionRect, SerializedProperty property, GUIContent label)
    {
        ...
        if (target != null)
        {
            ...
            Rect methodRect = new Rect(positionRect.x, targetRect.max.y + EditorGUIUtility.standardVerticalSpacing, positionRect.width, EditorGUIUtility.singleLineHeight);

            // Method select button
            position = EditorGUI.PrefixLabel(methodRect, GUIUtility.GetControlID(FocusType.Passive), new GUIContent(dynamic ? "Method (dynamic)" : "Method"));
            ...
            if (activeMethod != null && !dynamic)
            {
                ...
                Rect argumentRect = new Rect(positionRect.x, methodRect.max.y + EditorGUIUtility.standardVerticalSpacing, positionRect.width, EditorGUIUtility.singleLineHeight);

                string[] types = new string[argumentProperties.arraySize];
                for (int i = 0; i < types.Length; i++)
                {
                    SerializedProperty argumentProperty = argumentProperties.FindPropertyRelative("Array.data[" + i + "]");
                    ...
                    switch ((Argument.ArgumentType)argumentProperty.FindPropertyRelative("Type").enumValueIndex)
                    {
                        case Argument.ArgumentType.Bool:
                            EditorGUI.PropertyField(argumentRect, argumentProperty.FindPropertyRelative("BoolValue"), argLabel);
                            break;
                        case Argument.ArgumentType.Int:
                            EditorGUI.PropertyField(argumentRect, argumentProperty.FindPropertyRelative("IntValue"), argLabel);
                            break;
                        case Argument.ArgumentType.Float:
                            EditorGUI.PropertyField(argumentRect, argumentProperty.FindPropertyRelative("FloatValue"), argLabel);
                            break;
                        case Argument.ArgumentType.String:
                            EditorGUI.PropertyField(argumentRect, argumentProperty.FindPropertyRelative("StringValue"), argLabel);
                            break;
                        case Argument.ArgumentType.Object:
                            EditorGUI.PropertyField(argumentRect, argumentProperty.FindPropertyRelative("ObjectValue"), argLabel);
                            break;
                    }
        ...
        {
            Rect helpBoxRect = new Rect(positionRect.x + 8, targetRect.max.y + EditorGUIUtility.standardVerticalSpacing, positionRect.width - 16, EditorGUIUtility.singleLineHeight);
    ...
    string PrettifyMethod(MethodInfo methodInfo)
    {
        ...
        return GetTypeName(methodInfo.ReturnParameter.ParameterType) + " " + methodInfo.Name + "(" + parameterNames + ")";
    }
    ...
    private Type[] GetArgumentTypes(SerializedProperty argProp)
    {
        ...
        {
            types[i] = Argument.RealType((Argument.ArgumentType)argProp.FindPropertyRelative("Array.data[" + i + "].ArgumentTypes").enumValueIndex);
        }
        return types;
    }
    ...
    private void SetMethod(SerializedProperty property, UnityEngine.Object target, MethodInfo methodInfo, bool dynamic)
    {
        ...
        {
            argumentProperty.FindPropertyRelative("Array.data[" + i + "].ArgumentTypes").enumValueIndex = (int)Argument.FromRealType(parameters[i].ParameterType);
        }
        property.FindPropertyRelative("m_Dirty").boolValue = true;
