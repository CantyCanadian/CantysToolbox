...
public class SplineBezier : MonoBehaviour
{
    ...
    public void Reset()
    {
        m_Points = new Vector3[] { new Vector3(1.0f, 0.0f, 0.0f), new Vector3(2.0f, 0.0f, 0.0f), new Vector3(3.0f, 0.0f, 0.0f), new Vector3(4.0f, 0.0f, 0.0f) };
    ...
    public Vector3 GetVelocity(float time)
    {
        ...
        return transform.TransformPoint(GetFirstDerivative(m_Points[i], m_Points[i + 1], m_Points[i + 2], m_Points[i + 3], time)) - transform.position;
    }
    ...
    public Vector3 GetPoint(float time)
    {
        ...
        return transform.TransformPoint(GetPoint(m_Points[i], m_Points[i + 1], m_Points[i + 2], m_Points[i + 3], time));
    }
    ...
    public static Vector3 GetPoint(Vector3 point0, Vector3 point1, Vector3 point2, Vector3 point3, float time)
    {
        ...
        return
            Mathf.Pow(oneMinusT, 3.0f) * point0 + 3.0f * Mathf.Pow(oneMinusT, 2.0f) * time * point1 + 3.0f * oneMinusT * Mathf.Pow(time, 2.0f) * point2 + Mathf.Pow(time, 3.0f) * point3;
    }

    public static Vector3 GetFirstDerivative(Vector3 point0, Vector3 point1, Vector3 point2, Vector3 point3, float time)
    {
        time = Mathf.Clamp01(time);
        float oneMinusT = 1f - time;
        return 3f * oneMinusT * oneMinusT * (point1 - point0) + 6f * oneMinusT * time * (point2 - point1) + 3f * time * time * (point3 - point2);
    }
...
public class SplineBezierInspector : Editor
{
    ...
    private void DrawSelectedPointInspector()
    {
        ...
        EditorGUI.BeginChangeCheck();
        SplineBezier.BezierControlPointMode mode = (SplineBezier.BezierControlPointMode)EditorGUILayout.EnumPopup("Mode", m_Spline.GetControlPointMode(m_SelectedIndex));
        if (EditorGUI.EndChangeCheck())
        {
